...
// 				validatorgroup : "1",
			...
			$("#idvalue").formValidator({
				ajax:true,
				empty : false,
				onShow : "请输入你的ID不可以为空哦",
				onFocus : "格式例如：BBA778*888，34位编码",
				onCorrect : "谢谢你的合作"
			}).inputValidator({
				min : 34,
				max : 34,
				type : "size",
				onError : "ID长度必须是34位，请确认"
			}).regexValidator({
				regExp : [ "username" ],
				dataType : "enum",
				onError : "你输入的ID格式不正确,字母数字组成"
			}).ajaxValidator({
				dataType : "json",
				async : true,
				type : "post",
				url : basePath + "/buzid/validatorID.htm",
