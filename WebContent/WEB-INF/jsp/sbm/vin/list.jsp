...
<head lang="zh-CN">
    ...
    <script type="text/javascript">
		...
		function showDeafaultTable(pageNo, pageSize) {
			var url = basePath + "/buzvin/showPage.htm?temp=" + new Date().getTime();
			showTable(pageNo, pageSize, url);
		}
		
		function showTable(pageNo, pageSize, url) {
			if (!url) {
				url = basePath + "/buzvin/showPage.htm?temp=" + new Date().getTime();
			}
//	 		url += getQueryMap();
		...
		$(function(){
			var douc = window.parent.frames["side"].document.getElementById("vinlist");
			douc.className="ed";
			var douc = window.parent.frames["side"].document.getElementById("vinadd");
			douc.className="";
		...
		function deleted(vinId){
			$.ajax({
				type: "post",
			    dataType: "json",
			    async:false,
			    url: "<c:url value='/buzvin/deleteVin.htm'/>?vin.id="+vinId,
...
<div class="container">
    <div class="breadNav">
        <ul class="clear">
            ...
            <li class="bread-home"><a href="javascript:void(0);"><font>&gt;&gt;&nbsp;</font>VIN列表</a></li>
        </ul>
    </div>
    <!-- /breadNav -->
    <div class="searchBar clear">
        <div class="label-form">
            <label for="num" class="lb-label">VIN</label>
            <input class="lb-input" type="text" id="num" name="queryMap.vin"/>
        </div>
        <div class="label-form">
			...
			<![CDATA[
					{#template MAIN}
						<table class="u-tab">
							...
							<tr>
								...
								<th align="center" width="50%">VIN</th>
					...
					{#template ROW}
						<tr>
							...
							<td align="center">
								<a href="#" onClick="deleteVin({$T.id})">删除</a>
							</td>
						</tr>
					{#/template ROW}
			]]>
