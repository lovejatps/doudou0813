...
<head lang="zh-CN">
	...
	<script type="text/javascript">
		...
		function showDeafaultTable(pageNo, pageSize) {
			var url = basePath + "/buzsn/showPage.htm?temp=" + new Date().getTime();
			showTable(pageNo, pageSize, url);
		}
		
		function showTable(pageNo, pageSize, url) {
			if (!url) {
				url = basePath + "/buzsn/showPage.htm?temp=" + new Date().getTime();
			}
//	 		url += getQueryMap();
		...
		$(function(){
			var douc = window.parent.frames["side"].document.getElementById("snlist");
			douc.className="ed";
			var douc = window.parent.frames["side"].document.getElementById("snadd");
			douc.className="";
		...
		function deleted(snId){
			$.ajax({
				type: "post",
			    dataType: "json",
			    async:false,
			    url: "<c:url value='/buzsn/deleteSn.htm'/>?sn.id="+snId,
...
<div class="container">
    <div class="breadNav">
        <ul class="clear">
            ...
            <li class="bread-home"><a href="javascript:void(0);"><font>&gt;&gt;&nbsp;</font>SN管理</a></li>
            <li class="bread-home"><a href="javascript:void(0);"><font>&gt;&gt;&nbsp;</font>SN列表</a></li>
        </ul>
    </div>
    <!-- /breadNav -->
    <div class="searchBar clear">
        <div class="label-form">
            <label for="num" class="lb-label">SN</label>
            <input class="lb-input" type="text" id="num" name="queryMap.sn"/>
        </div>
        <div class="label-form">
