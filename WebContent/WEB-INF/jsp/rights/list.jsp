...
<head lang="zh-CN">
	...
	<script type="text/javascript">
		...
		function showDeafaultTable(pageNo, pageSize) {
			var url = basePath + "/rights/showPage.htm?temp=" + new Date().getTime();
			showTable(pageNo, pageSize, url);
		}
		
		function showTable(pageNo, pageSize, url) {
			if (!url) {
				url = basePath + "/rights/showPage.htm?temp=" + new Date().getTime();
			}
//	 		url += getQueryMap();
		...
		$(function(){
			 var douc = window.parent.frames["side"].document.getElementById("rightlist");
			douc.className="ed";
			var douc = window.parent.frames["side"].document.getElementById("rightadd");
			douc.className="";
			...
			showDeafaultTable(1, 20);
		});
	 
		function deleteSn(aniId){
			if(aniId ==-1){
				alert("该记录不可删除！");				
			}else{
				...
				$.ajax({
					    ...
					    dataType: "json",
					    async:false,
					    url: "<c:url value='/rights/selectProduce.htm'/>?prod.id="+aniId,
					    success: function (data) {
							if(data==1){
								layer.alert("家厂已经存在关联，不能删除", 0);
							}else{							
								$.layer({
								    shade: [0],
								    area: ['auto','auto'],
		...
		function deleted(id){
			$.ajax({
			    ...
			    dataType: "json",
			    async:false,
			    url: "<c:url value='/rights/deleteProduce.htm'/>?prod.id="+id,
			    success: function (data) {
					if(data==1){
						layer.alert("成功删除！", 0);
						showDeafaultTable(1, 10);
...
<div class="container">
    <div class="breadNav">
        <ul class="clear">
            ...
            <li class="bread-home"><a href="javascript:void(0);"><font>&gt;&gt;&nbsp;</font>厂家列表</a></li>
        </ul>
    </div>
    <!-- /breadNav -->
    <div class="searchBar clear">
        <div class="label-form">
            <label for="num" class="lb-label">厂家名称</label>
            <input class="lb-input" type="text" id="num" name="queryMap.name"/>
        </div>
        <div class="label-form">
			...
			<![CDATA[
					{#template MAIN}
						<table class="u-tab">
							...
							<tr>
								...
								<th align="center" width="30%">厂家名称</th>
								<th align="center" >厂家地址</th>
					...
					{#template ROW}
						<tr>
							...
							<td align="center">
								<a href="#" onClick="deleteSn({$T.id})">删除</a>
							</td>
						</tr>
					{#/template ROW}
			]]>
