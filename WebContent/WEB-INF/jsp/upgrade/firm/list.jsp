...
<head lang="zh-CN">
	...
	<script type="text/javascript">
		...
		function showDeafaultTable(pageNo, pageSize) {
			var url = basePath + "/upgrade/firm/showFirm.htm?temp=" + new Date().getTime();
			showTable(pageNo, pageSize, url);
		}
		
		function showTable(pageNo, pageSize, url) {
			if (!url) {
				url = basePath + "/upgrade/firm/showFirm.htm?temp=" + new Date().getTime();
			}
//	 		url += getQueryMap();
		...
		$(function(){
			var douc = window.parent.frames["side"].document.getElementById("firmlist");
			douc.className="ed";
			var douc = window.parent.frames["side"].document.getElementById("firmadd");
			douc.className="";
		...
		function deleted(sysId){
			$.ajax({
			    ...
			    dataType: "json",
			    async:false,
			    url: "<c:url value='/upgrade/firm/deleteFirm.htm'/>?firmId="+sysId,
...
<div class="container">
    ...
    <div class="searchBar clear">
        <div class="label-form">
            <label for="user-car" class="lb-label">固件型号</label>
			...
			<![CDATA[
					{#template MAIN}
						<table 	class="u-tab">
							...
							<tr>
								...
								<th align="center">固件型号</th>
								<th align="center" width="20%">所属厂家</th> 
					...
					{#template ROW}
						<tr>
							...
							<td align="center">
								<a href="#" onClick="deleteFirm({$T.id})">删除</a>
							</td>
						</tr>
					{#/template ROW}
			]]>
